...
parser.add_argument('--pretrained-disp', dest='pretrained_disp', metavar='PATH', help='path to pre-trained dispnet model')
parser.add_argument('--pretrained-pose', dest='pretrained_pose', metavar='PATH', help='path to pre-trained Pose net model')
...
def main():
    ...
    weights = torch.load('dispnet_checkpoint.pth.tar')
    disp_net.load_state_dict(weights['state_dict'])
    print("=> using pre-trained weights for PoseResNet")
    weights = torch.load('exp_pose_checkpoint.pth.tar')
...
def validate_depth_with_gt(args, val_loader, disp_net, epoch, logger, output_writers=[]):
    ...
    for i, (tgt_img, tgt_img_loss, depth) in enumerate(val_loader):
        ...
        # compute output
        output_disp = disp_net(tgt_img)   # tensor
        output_depth = 1/output_disp     # tensor
...
def compute_depth(disp_net, tgt_img, ref_imgs):
    # print(type(tgt_img))   # tensor
    disps = disp_net(tgt_img)   # list
    tgt_depth = [1/disp for disp in disps]  # list
